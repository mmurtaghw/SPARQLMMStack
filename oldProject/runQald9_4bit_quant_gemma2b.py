...
# Load the QALD-9 Plus dataset
with open('QALD_9_plus/data/qald_9_plus_test_dbpedia.json', 'r') as file:
    qald_data = json.load(file)

# Number of cases to process (set to None to process all cases)
num_cases = None  # Change this to an integer (e.g., 10) to limit the number of cases processed

# Prepare results storage
results = []

# Experiment parameters
models = ["google/gemma-2-2b-it"]  # Select based on availability
...
results_df.to_csv("qald_9_experiment_report.csv", index=False)
print("\nExperiment report saved as 'qald_9_experiment_report_Gemma2b_4bit.csv'")
